...
def get_pitch_parselmouth(wav_data, hop_size=512, audio_sample_rate=44100, interp_uv=True):
...
def get_pitch_rmvpe(wav_data, hop_size, audio_sample_rate, interp_uv=True):
    ...
    if rmvpe is None:
        ...
        rmvpe = RMVPE(pathlib.Path(__file__).parent /'model'/' rmvpe' / 'model.pt')
...
def get_pitch(algorithm, wav_data,  hop_size=512, audio_sample_rate=44100, interp_uv=True):
